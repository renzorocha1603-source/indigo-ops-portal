...
LOT_DATA = [
    ...
    {"cmo": "M020", "name": "2984 Taschereau Greenfield Park", "client": "Northwest Healthcare Properties"},
...
def create_vertical_pdf(df):
    ...
    elements.append(Paragraph("<b>Indigo Park - City Reporting Matrix Report</b>", styles['Title']))
    ...
    for index, row in df.iterrows():
        ...
        table.setStyle(TableStyle([('GRID', (0, 0), (-1, -1), 0.5, colors.black), ('VALIGN', (0, 0), (-1, -1), 'TOP'), ('BACKGROUND', (0, 0), (0, -1), colors.whitesmoke)]))
...
LANGS = {
    "English": {"title": "City Reporting Matrix", "tab1": "Form", "tab2": "History", "cmo": "Lot / Client", "year": "Year", "month": "Month", "sign": "Full Name", "submit": "Submit", "comm": "Comment", "attest": "Verified", "dl_excel": "📥 Excel", "dl_pdf": "📥 PDF"},
    "Français": {"title": "Matrice de Rapports", "tab1": "Formulaire", "tab2": "Historique", "cmo": "Lot / Client", "year": "Année", "month": "Mois", "sign": "Nom complet", "submit": "Soumettre", "comm": "Commentaire", "attest": "Vérifié", "dl_excel": "📥 Excel", "dl_pdf": "📥 PDF"}
}
...
with tab1:
    ...
    if st.button(T["submit"]):
        if not sig: st.error("Name required")
            ...
            row = {"Date": datetime.datetime.now().strftime("%Y-%m-%d"), "Year": year, "Month": month, "CMO": cmo, "User": sig, "Score": pct}
...
with tab2:
    ...
    if not df.empty:
        ...
        buff = io.BytesIO()
        df.T.to_excel(buff)
        c1.download_button(T["dl_excel"], buff.getvalue(), "report.xlsx")
        c2.download_button(T["dl_pdf"], create_vertical_pdf(df), "report.pdf")
